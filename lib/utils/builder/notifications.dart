...
class NotificationsBuilder {
  static NotificationsBuilder instance = new NotificationsBuilder.internal();

  NotificationsBuilder.internal();

  factory NotificationsBuilder() => instance;
  ...
  Future<void> showInboxNotification(
      {required String inboxChannelId,
      required String header,
      required String title,
      required String body,
      required String inboxChannelName,
      required String inboxChannelDescription}) async {
    final List<String> lines = <String>['<b>${title}</b>', '<i>${body}</i>'];
    final InboxStyleInformation inboxStyleInformation = InboxStyleInformation(
        lines,
        htmlFormatLines: true,
        contentTitle: '<b>${header}</b>    ${title}',
        htmlFormatContentTitle: true,
        summaryText: 'summary <i>${body}</i>',
        htmlFormatSummaryText: true);
    final AndroidNotificationDetails androidPlatformChannelSpecifics =
        AndroidNotificationDetails(
            inboxChannelId, inboxChannelName, inboxChannelDescription,
  ...
  Future<void> showMessagingNotification() async {
    ...
    final List<Message> messages = <Message>[
      ...
      Message('Lunch?', DateTime.now().add(const Duration(minutes: 10)), null,
          ...
          dataUri: "https://www.hijozaty.com/profile_images/no_image.png"),
